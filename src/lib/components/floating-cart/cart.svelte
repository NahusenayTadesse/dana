...
{:else}
	<!--
		The fixed placement moved from the trigger to this column so the WhatsApp
		button can stack above the cart by flow rather than by a hardcoded offset
		that would drift the moment the cart button changed size.
	-->
	<div class="fixed right-6 bottom-24 z-50 flex flex-col items-center gap-3 md:bottom-6">
		<FloatingContact
			href={siteSetting('social_whatsapp')}
