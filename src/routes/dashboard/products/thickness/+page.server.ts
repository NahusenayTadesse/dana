	...
	add: async ({ request, locals }) => {
		...
		try {
			...
			if (err.code === 'ER_DUP_ENTRY') {
				setError(form, 'value', 'Thickness value or unit already exists.');
				setError(form, 'unit', 'Thickness value  or unit already exists.');
			}
			return message(form, {
				type: 'error',
				text:
					err.code === 'ER_DUP_ENTRY'
						? 'Thickness value and unit already exists. Please choose another one.'
						: err.message
			});
		}
	},
	edit: async ({ request, locals }) => {
		...
		try {
			...
			return message(form, {
				...
				text:
					err.code === 'ER_DUP_ENTRY'
						? 'Thickness value or unit is already taken. Please choose another one.'
						: err.message
			});
		}
	}
