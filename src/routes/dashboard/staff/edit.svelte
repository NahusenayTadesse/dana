<script lang="ts">
	...
	import type { EditStaff } from './schema';
	...
	let {
		...
		row,
		icon = false
	}: { data: SuperValidated<EditStaff>; row: StaffRow; icon?: boolean } = $props();

	let open = $state(false);

	const { form, errors, enhance, delayed, message, allErrors } = untrack(() =>
		superForm(data, { resetForm: false, dataType: 'json', id: `staff-${row.id}` })
	);
...
<DialogComp
	...
	size="md"
>
	<form action="?/edit" method="post" use:enhance id="staff-edit-{row.id}" class="flex flex-col gap-3">
		...
		<Button type="submit" class="mt-2" form="staff-edit-{row.id}">
			{#if $delayed}
