<script lang="ts">
	...
	import { page as sveltePage, navigating } from '$app/state';
	...
	let minThick = $state(Number(sveltePage.url.searchParams.get('minThick')) || THICKNESS_MIN);
	let maxThick = $state(Number(sveltePage.url.searchParams.get('maxThick')) || THICKNESS_MAX);
	...
	// Every filter path funnels through here, and every one of them used to fire
	// its own goto() immediately: a keystroke in the search box was a server
	// round-trip, and ticking three colours was three sequential page loads.
	// Coalescing them means a burst of taps (or typing) resolves to one request.
	...
	// Enter shouldn't make you wait out the debounce.
	function handleSearchKeydown(e: KeyboardEvent) {
		if (e.key === 'Enter') updateFilters({ search: searchQuery }, { immediate: true });
	}
	...
	const goToPage = (p: number) => updateFilters({ page: p }, { immediate: true });

	// Breakpoint handled in CSS rather than JS: the old `isMobile()` returned
	// false during SSR (so phones got the sidebar open on first paint) and never
	// re-evaluated, which could leave the panel expanded with its toggle button
	// unrendered and no way to close it. Below lg this boolean drives the panel;
...
<div class="min-h-screen antialiased transition-colors duration-300">
	<!-- top-[--header-h], not top-0: the site header is sticky top-0 z-50, so a
         second bar pinned to 0 slides underneath it and the search box, result
         count and Clear-filters button disappear on scroll. -->
	<header
		class="sticky top-[var(--header-h)] z-40 border-b border-border/80 bg-background/75 shadow-xs backdrop-blur-md"
	>
			...
			<div class="mb-4 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
				<div>
					...
					<p class="mt-1 text-xs text-muted-foreground sm:text-sm">
						{m.shop_total_items({ count: data?.pagination?.totalCount ?? 0 })}
					</p>
				</div>
	...
	<main class="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
		<div class="grid grid-cols-1 gap-8 lg:grid-cols-4">
			...
			<aside class="{showFilter ? 'block' : 'hidden'} lg:col-span-1 lg:block">
				<div
					class="sticky top-[calc(var(--header-h)+8rem)] space-y-6 rounded-2xl border border-border/80 bg-card/40 p-5 shadow-sm backdrop-blur-lg"
				>
					...
					<div class="space-y-3 border-b border-border/60 pb-4">
						...
						<div class="relative h-5 select-none">
							...
							<input
								...
								aria-label={m.shop_min_thickness_aria()}
							/>
							<input
								type="range"
								min={THICKNESS_MIN}
								...
								onchange={applyThicknessFilter}
								class="range-thumb pointer-events-none absolute top-1/2 h-5 w-full -translate-y-1/2 appearance-none bg-transparent"
								aria-label={m.shop_max_thickness_aria()}
							/>
						</div>
					</div>
					...
					<!-- Categories -->
					<div
						class="max-h-55 scrollbar-none space-y-2.5 overflow-y-auto border-b border-border/60 pb-4"
					>
						<div class="flex items-center gap-2">
							<Layers class="size-3.5 text-muted-foreground" />
							<h4 class="text-xs font-bold tracking-wider text-muted-foreground uppercase">
								{m.shop_product_classifications()}
							</h4>
						</div>
						...
						<div class="flex flex-wrap gap-2 pt-1">
							{#each availableWidths as w (w.id)}
								...
								<button
									...
									class="rounded-full border px-2.5 py-1 text-xs font-medium transition-all duration-150
                                        {isSelected
						...
						<div class="flex flex-wrap gap-2 pt-1">
							{#each availableColors as color (color.id)}
								...
								<button
									...
									class="flex items-center gap-1.5 rounded-full border px-2.5 py-1 text-xs font-medium transition-all duration-150
                                        {isSelected
										? 'border-primary bg-primary text-primary-foreground shadow-xs'
										: 'border-border bg-card text-foreground/80 hover:bg-accent'}"
								>
									{#if color.hexValue}
										<span
											class="h-2.5 w-2.5 shrink-0 rounded-full border border-foreground/10"
			...
			<div
				class="space-y-8 transition-opacity duration-150 lg:col-span-3 {navigating.to
